...
{
    (void)user_data;
    stdf_mvc_payload_ai_detection_t *det = (stdf_mvc_payload_ai_detection_t *)data->ptr;
    if (!det) {
        return;
    }
    ...
    stdf_mvc_payload_gimbal_target_t tgt = {
...
static void on_ai_tracking(void *user_data, const stdf_mvc_signal_data_t *data)
{
    ...
    stdf_mvc_payload_gimbal_motion_event_t evt = {
        .motion = (trk->state == STDF_MVC_PAYLOAD_AI_TRACK_LOCKED)
                  ? STDF_MVC_PAYLOAD_GIMBAL_MOTION_MOVING
                  : STDF_MVC_PAYLOAD_GIMBAL_MOTION_STOP,
    };
...
void stdf_app_gimbal_tick(void)
{
    ...
    stdf_mvc_payload_gimbal_pose_t pose = {
