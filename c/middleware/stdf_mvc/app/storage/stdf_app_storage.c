...
void stdf_app_storage_tick(void)
{
    ...
    stdf_mvc_payload_sd_state_event_t sd_state = {
        .state = hal->sd_inserted ? STDF_MVC_PAYLOAD_SD_STATE_MOUNTED : STDF_MVC_PAYLOAD_SD_STATE_MISSING,
    };
    strncpy(sd_state.label, "sd", sizeof(sd_state.label) - 1);
    stdf_mvc_signal_data_t p = { .ptr = &sd_state };
    stdf_mvc_subject_emit(STDF_MVC_SUBJECT_SD_CARD_STATE, &p);

    stdf_mvc_payload_sd_storage_t sd_storage = {
