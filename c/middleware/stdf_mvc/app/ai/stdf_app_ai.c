...
static stdf_mvc_payload_ai_tracking_state_t s_tracking_state = STDF_MVC_PAYLOAD_AI_TRACK_LOST;
...
void stdf_app_ai_tick(void)
{
    ...
    if ((s_tick_count % 30) == 1) {
        ...
        stdf_mvc_payload_ai_tracking_state_t prev_state = s_tracking_state;

        if (!s_has_track && (rand() % 3) == 0) {
            s_has_track = 1;
            s_track_id  = 100 + (rand() % 100);
            s_tracking_state = STDF_MVC_PAYLOAD_AI_TRACK_LOCKED;
        } else if (s_has_track && (rand() % 20) == 0) {
            s_has_track = 0;
            s_tracking_state = STDF_MVC_PAYLOAD_AI_TRACK_LOST;
        }

        if (prev_has_track != s_has_track || prev_state != s_tracking_state) {
            stdf_mvc_payload_ai_tracking_t trk = {
                .state        = s_tracking_state,
                .track_id     = s_track_id,
                .object_class = STDF_MVC_PAYLOAD_AI_OBJ_PERSON,
            };
    ...
    if (s_has_track) {
        stdf_mvc_payload_ai_detection_t det = {
            .object_class = STDF_MVC_PAYLOAD_AI_OBJ_PERSON,
            .track_id     = s_track_id,
