...
#define STDF_MVC_LOG_EMIT(...)         do { } while (0)
#define STDF_MVC_EMIT_ASSERT(cond)
...
void stdf_mvc_emit_subsystem_cleanup(void)
{
    uint32_t i;
    for (i = 0; i < STDF_MVC_SUBJECT_COUNT; i++) {
        if (s_heads[i]) {
                                ...
                                void                 *user_data)
{
    ...
    if (!node) {
        stdf_mvc_port_exit_critical();
        stdf_mvc_port_log(STDF_MVC_LOG_LVL_ERROR,
                          "pool full, subscribe [%u] %s failed",
    ...
    stdf_mvc_port_log(STDF_MVC_LOG_LVL_DEBUG,
                      "subscribe [%u] %s -> count=%u",
                      (unsigned)id, stdf_mvc_subject_str(id),
                      (unsigned)s_counts[id]);
#endif
...
{
    stdf_mvc_port_log(STDF_MVC_LOG_LVL_INFO, "=== stdf_mvc subjects ===");
    for (stdf_mvc_subject_id_t id = 0; id < STDF_MVC_SUBJECT_COUNT; id++) {
        if (s_counts[id] > 0) {
            stdf_mvc_port_log(STDF_MVC_LOG_LVL_INFO,
                              "  [%u] %s: %u subs",
                              (unsigned)id,
                              stdf_mvc_subject_str(id),
                              (unsigned)s_counts[id]);
        }
    }
    stdf_mvc_port_log(STDF_MVC_LOG_LVL_INFO, "=== pool: %u/%u used ===",
