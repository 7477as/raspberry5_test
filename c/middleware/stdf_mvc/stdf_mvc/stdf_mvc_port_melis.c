...
static void melis_log(stdf_mvc_log_lvl_t lvl, const char *fmt, va_list ap)
{
    static const char *tag[] = {"[DBG]", "[INF]", "[WRN]", "[ERR]"};
    printf("[STDF_MVC]%s ", tag[lvl]);
    vprintf(fmt, ap);
    printf("\n");
}

static void melis_assert_fail(const char *expr, const char *file, int line)
{
    printf("[STDF_MVC][ASSERT] %s:%d %s\n", file, line, expr);
    while (1) { }
}
