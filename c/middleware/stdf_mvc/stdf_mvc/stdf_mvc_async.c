...
#define STDF_MVC_LOG_ASYNC(...)         do { } while (0)
#define STDF_MVC_ASYNC_ASSERT(cond)
...
void stdf_mvc_async_subsystem_cleanup(void)
{
    pthread_mutex_lock(&s_async.mtx);
    s_async.running = 0;
                                  ...
                                  const stdf_mvc_signal_data_t  *data)
{
    ...
    if (ret != 0) {
        stdf_mvc_port_log(STDF_MVC_LOG_LVL_WARN,
                          "emit_async [%u] %s: queue full, drop",
                          (unsigned)id, stdf_mvc_subject_str(id));
        stdf_mvc_port_mem_free(msg);
        return -3;
    }
