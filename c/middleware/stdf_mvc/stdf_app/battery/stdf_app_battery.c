...
{
    (void)user_data;
    stdf_app_battery_level_t *lvl = (stdf_app_battery_level_t *)data->ptr;
    if (!lvl) {
        return;
    }
    if (lvl->level_pct <= 15 && (s_tick_count - s_last_low_battery_warn_tick) > 200) {
        stdf_app_sys_err_t err = {
            .code = SYS_ERR_LOW_BATTERY,
            .detail = "battery <= 15%",
...
void stdf_app_battery_tick(void)
{
    ...
    stdf_app_battery_level_t lvl = { .level_pct = hal->battery_pct };
    stdf_mvc_signal_data_t p = { .ptr = &lvl };
    stdf_mvc_subject_emit(STDF_MVC_SUBJECT_BATTERY_LEVEL, &p);

    stdf_app_battery_voltage_t volt = { .voltage_mv = hal->battery_mv };
    p.ptr = &volt;
    stdf_mvc_subject_emit(STDF_MVC_SUBJECT_BATTERY_VOLTAGE, &p);

    stdf_app_battery_chg_event_t chg = {
        .state = (stdf_app_battery_chg_state_t)hal->battery_chg,
    };
    p.ptr = &chg;
