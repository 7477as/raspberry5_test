...
static void on_button_record(void *user_data, const stdf_mvc_signal_data_t *data)
{
    ...
    if (!btn || btn->id != BTN_RECORD || btn->event != BTN_EVT_PRESS) {
        return;
    }
    ...
    stdf_app_video_record_t rec = {
        .state       = s_is_recording ? VIDEO_STATE_RECORDING : VIDEO_STATE_IDLE,
...
void stdf_app_video_tick(void)
{
    ...
    if ((s_tick_count % 3) == 0) {
        ...
        stdf_app_video_frame_raw_t raw = {
        ...
        stdf_app_video_frame_encoded_t enc = {
            .data        = s_dummy_nalu,
            .size_bytes  = sizeof(s_dummy_nalu),
