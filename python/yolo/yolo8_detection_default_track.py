...
def mode_photo(model, cap):
    ...
    print("\n按任意键关闭图片窗口并退出...")
    cv2.imshow("Object Detection - Photo Mode", annotated_frame)
    cv2.waitKey(0)
    cv2.destroyAllWindows()

def mode_video_without_tracking(model, cap):
    """
    模式 2a: 实时视频流物体检测（不使用跟踪）
    """
    print("\n启动实时视频流物体检测（无跟踪）...")
    ...
    while True:
        ...
        if obj_count > 0:
            ...
            class_name = result.names[class_id]
            print(f"\r检测到 {obj_count} 个物体 | 主要物体: {class_name} (ID:{class_id}) | 置信度: {confidence*100:.2f}% | FPS: {current_fps:.1f}", end="")
        else:
            print(f"\r未检测到物体 | FPS: {current_fps:.1f}", end="")
        ...
        # 显示画面
        cv2.imshow("Object Detection - Real-time (No Tracking)", annotated_frame)
...
def mode_video_with_tracking(model, cap, tracker_type='bytetrack'):
    ...
    print(f"\n启动实时视频流物体检测（{tracker_name} 跟踪）...")
    print(f"跟踪器特点:")
    for feature in tracker_features:
        print(f"  {feature}")
    print("\n提示: 在视频窗口处于焦点时，按下 'q' 键退出。")
    print("提示: 每个物体会被分配唯一ID，并持续跟踪")
    ...
    if tracker_type == 'bytetrack':
        ...
        window_title = f"Object Tracking with {tracker_name}"
    else:
        info_color = (255, 165, 0)  # 橙色
        window_title = f"Object Tracking with {tracker_name} (Higher Accuracy)"
    ...
    while True:
        ...
        if has_tracking and obj_count > 0:
            ...
            confidence = first_box.conf[0].item()
            class_name = result.names[class_id]
            print(f"\r[{tracker_name}] 跟踪中: {active_tracks} 个目标 | 检测: {obj_count} 个物体 | 主要: ID:{track_id} {class_name} | 置信度: {confidence*100:.2f}% | FPS: {current_fps:.1f} | 推理: {avg_inference_time*1000:.1f}ms", end="")
        elif obj_count > 0:
            print(f"\r[{tracker_name}] 检测到 {obj_count} 个物体（跟踪初始化中）| FPS: {current_fps:.1f}", end="")
        else:
            print(f"\r[{tracker_name}] 未检测到物体 | FPS: {current_fps:.1f}", end="")
...
def main():
    print("="*60)
    print("   YOLOv8 + RpiCam + 多跟踪器物体检测与跟踪系统")
    print("="*60)
    
    # 1. 加载检测模型
    try:
        # 使用检测模型（可以根据需要更换）
        model_path = 'models/yolov8n.pt'  # 轻量级模型，适合树莓派
        ...
        print(f"加载模型失败: {e}")
        print("请确保模型文件 'models/yolov8n.pt' 存在")
        print("可以运行: yolo detect train 或下载预训练模型")
        return
